      ...
      submit() {
        ...
        specs.forEach(({params}) => {
          params.forEach(p => {
            //给特有规格设置options
            if (!p.global) {
              let sku = this.skuTemplate.find(sku=>sku.k===p.k);
              console.log(sku.options.length);
              console.log(sku.k);
              if(sku.options&&sku.options.length>0)
              {
                p.options = sku.options;
              }

            }else
            {
              console.log("删除options属性");
              delete  p.options;
            }
          })
        })
    ...
    watch: {
      ...
      skuTemplate: {
        ...
        handler(val) {
          ...
          if (this.isEdit) {
            // 查询sku
            this.$http.get("/item/goods/skus?spuId="+this.goods.id).then(resp => {
              // 处理SKU
